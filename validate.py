...
def main() -> None:
    with open("crawler-user-agents.json") as f:
        json_data: list[UserAgent] = json.load(f)
    ...
    for pattern, count in pattern_counts.most_common():
        if count > 1:
            raise ValueError("Pattern {!r} appears {} times".format(pattern, count))
    ...
    for pattern, count in pattern_counts.most_common():
        if count > 1:
            raise ValueError(
                "Pattern {!r} is duplicated {} times with different capitalization".format(
                    pattern, count
                )
            )
    ...
    for entry in json_data:
        ...
        if re.search("[^\\\\]/", pattern):
            raise ValueError(
                "Pattern {!r} has an unescaped slash character".format(pattern)
            )
    ...
    for entry in json_data:
        ...
        if re.search("[^\\\\]\\.", pattern):
            raise ValueError(
                "Pattern {!r} has an unescaped dot character".format(pattern)
            )
    ...
    for entry in json_data:
        ...
        if "addition_date" in entry:
            if not re.match(r"\d{4}/\d{2}/\d{2}", entry["addition_date"]):
                raise ValueError(
                    "addition_date {!r} has invalid format".format(
                        entry["addition_date"]
                    )
                )
        ...
        JSON_SCHEMA["items"]["properties"]
        assert set(entry.keys()) <= set(
            JSON_SCHEMA["items"]["properties"].keys()
        ), "the entry contains unknown properties"
        ...
        if instances:
            ...
            for instance in instances:
                ...
                if not re.search(pattern, instance):
                    raise ValueError(
                        "Pattern {!r} misses instance {!r}".format(pattern, instance)
                    )
